...
<div class="col-sm-4 mb-5">
 ...
 if($result->num_rows > 0){
  while($row = $result->fetch_assoc()){
   ...
   echo 'Serviço ID : '. $row['request_id'];
   echo '</div>';
   echo '<div class="card-body">';
   echo '<h5 class="card-title">Serviços info : ' . $row['request_info'] . '</h5>';
   echo '<p class="card-text">' . $row['request_desc'] . '</p>';
   echo '<p class="card-text">Request Date: ' . $row['request_date'] . '</p>';
   echo '<div class="float-right">';
   echo '<form action="" method="POST"> <input type="hidden" name="id" value='. $row["request_id"] .'><input type="submit" class="btn btn-danger mr-3" name="view" value="Ver"><input type="submit" class="btn btn-secondary" name="close" value="Fechar"></form>';
   echo '</div>' ;
   echo '</div>' ;
   echo'</div>';
  }
  ...
  <h4 class="alert-heading">Tudo Feito!</h4>
  <p>Todas as vagas foram preenchida.</p>
  <hr>
  <h5 class="mb-0">Sem vagas no momento</h5>
</div>';
 }
